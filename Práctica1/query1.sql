WITH flight_times
     ...
     AS (SELECT tf.ticket_no,
         ...
         FROM   ticket_flights tf
                JOIN flights f
                  ON tf.flight_id = f.flight_id
                JOIN flight_times ft
                  ON tf.ticket_no = ft.ticket_no
                     AND f.scheduled_departure = ft.first_dep_time),
     last_flight
     AS (SELECT tf.ticket_no,
                f.arrival_airport
         FROM   ticket_flights tf
