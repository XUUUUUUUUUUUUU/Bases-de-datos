...
ORDER BY i.ticket_no ASC;",
    ...
    printf("book_ref = ");
    fflush(stdout);
    while (scanf("%s", book_ref) != EOF)
    {
        book_ref[strcspn(book_ref, "\n")] = 0;
        SQLBindParameter(stmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, 6, 0, book_ref, (SQLINTEGER)sizeof(book_ref), NULL);
        ...
        {
            printf("%s flight_id = %s scheduled_departure= %s seat_no = %s\n", passenger_name, flight_id, scheduled_departure, assigned_seat_no);
        }
