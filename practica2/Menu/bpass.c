...

{
    ...
    *n_choices = 0;
    ...
    SQLPrepare(stmt, (SQLCHAR *)"WITH bookings_ticket AS ( "
                                ...
                                "), "
                                "available_seats AS ( "
                                "  SELECT s.seat_no, s.aircraft_code, f.flight_id, "
                                ...
                                "  SELECT bp.flight_id, COALESCE(MAX(bp.boarding_no), 0) AS max_boarding_no "
                                "  FROM boarding_passes bp GROUP BY bp.flight_id "
                                "), "
                                "ticket_to_allocate AS ( "
                                "  SELECT bt.ticket_no, bt.flight_id, avs.seat_no AS assigned_seat_no, "
                                ...
                                "  SELECT tta.ticket_no, tta.flight_id, tta.new_boarding_no, tta.assigned_seat_no "
                                "  FROM ticket_to_allocate tta "
                                "  ON CONFLICT (flight_id, seat_no) DO NOTHING "
                                "  RETURNING ticket_no, flight_id, boarding_no, seat_no "
                                ") "
                                "SELECT SUBSTRING(t.passenger_name FROM 1 FOR 20) AS passenger_name, "
                                "       i.flight_id, f.scheduled_departure, i.seat_no "
                                "FROM ins i "
    ...
    if (row == 0)
    {
        ...
        SQLPrepare(stmt, (SQLCHAR *)"SELECT 1 FROM bookings WHERE book_ref = ?;", SQL_NTS);
        ind = SQL_NTS;
        SQLBindParameter(stmt, 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR,
                         (SQLULEN)strlen(bookID), 0, (SQLPOINTER)bookID, 0, &ind);
        ...
        if (max_rows > 0)
        {
            if (ret == SQL_NO_DATA)
            {
                snprintf((*choices)[0], (size_t)max_length,
                         "El book_ref=%s no existe", bookID);
            }
            else
            {
                snprintf((*choices)[0], (size_t)max_length,
                         "Ya se ha asignado la tarjeta de embarque para book_ref=%s", bookID);
            }
            (*choices)[0][max_length - 1] = '\0';
            *n_choices = 1;
        }
