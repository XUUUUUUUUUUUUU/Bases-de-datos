...
"ORDER BY total_duration;", SQL_NTS);
    ...
    SQLBindCol(stmt, 8, SQL_C_CHAR, code1, sizeof(code1), NULL);
    SQLBindCol(stmt, 9, SQL_C_CHAR, depart1, sizeof(depart1), NULL);
    SQLBindCol(stmt, 10, SQL_C_CHAR, arrival1, sizeof(arrival1), NULL);
    SQLBindCol(stmt, 11, SQL_C_CHAR, second_id, sizeof(second_id), NULL);
    SQLBindCol(stmt, 12, SQL_C_CHAR, code2, sizeof(code2), NULL);
    ...
    while (row < max_rows && SQL_SUCCEEDED(ret = SQLFetch(stmt))) {
      ...
      if (strcmp((char*)connections, "1") == 0)
      {
        ...
        "first_flight_id:%-4s first_aricraft_code:%-3s sheduled_departure:%-20s sheduled_arrival:%-20s second_flight_id:%s second_aircraft_code:%s sheduled_departure:%s sheduled_arrival:%s", 
      ...
      {
        snprintf((*choices)[row],max_length,"%-4s  %-20s  %-20s  %s  %-3s  %-8s  \t "
        "first_flight_id:%-4s first_aricraft_code:%-3s sheduled_departure:%-20s sheduled_arrival:%-20s no hay segundo vuelos", 
        first_flight,scheduled_departure,scheduled_arrival,connections,free_seats,total_duration,
